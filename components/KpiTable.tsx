            ...
            <tr>
              {['date', 'type', 'metric', 'quantity', 'notes', 'link'].map((key) => (
                <th
                  ...
                  className="cursor-pointer select-none px-6 py-4 text-[11px] font-semibold uppercase tracking-[0.2em] text-navy-500 transition hover:text-usace-blue dark:text-navy-200 dark:hover:text-white"
                >
                  <span className="flex items-center gap-1">
                    {key}
                    <span>{getSortIndicator(key as keyof KpiDataPoint)}</span>
                  </span>
                </th>
              ))}
            </tr>
          ...
          <tbody className="divide-y divide-white/30 bg-white/60 text-sm text-navy-600 backdrop-blur dark:divide-white/5 dark:bg-white/5 dark:text-navy-100">
            {sortedData.map((item) => (
              <tr key={item.id} className="transition hover:bg-usace-blue/10 hover:text-usace-blue dark:hover:bg-white/10">
                ...
                <td className="whitespace-nowrap px-6 py-4 text-sm capitalize">{item.type}</td>
                <td className="whitespace-nowrap px-6 py-4 text-sm font-semibold text-navy-900 dark:text-white">{item.metric}</td>
                <td className="whitespace-nowrap px-6 py-4 text-sm">{item.quantity.toLocaleString()}</td>
                ...
                <td className="whitespace-nowrap px-6 py-4 text-sm">
                  {item.link ? (
                    <a
                      ...
                      className="text-usace-blue hover:underline"
                      title={item.link}
                    >
