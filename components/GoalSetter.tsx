...
const GoalSetter: React.FC<GoalSetterProps> = ({ goals, onAddGoal, campaigns }) => {
  ...
  const allMetrics = useMemo(() => {
    ...
    Object.values(METRIC_OPTIONS).forEach((options) => {
      options.forEach((option) => uniqueMetrics.add(option));
    });
                    ...
                    <li key={goal.id} className="py-4">
                      <div className="flex flex-col gap-2">
                        ...
                        <p className="text-sm text-navy-600 dark:text-navy-200">
                          Target:{' '}
                          <span className="font-semibold text-navy-900 dark:text-white">{goal.target_value.toLocaleString()}</span>
                        </p>
